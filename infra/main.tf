provider "azurerm" {
  features {}
  use_cli         = true
  subscription_id = "4032eaff-94b6-4a78-a5fe-0a9d0d69e136"
}

provider "local" {
  # Required for local_file
}
...
resource "azurerm_network_security_rule" "allow_http2" {
  name                        = "Allow-HTTP2"
  priority                    = 110
  direction                   = "Outbound"
  access                      = "Allow"
...
resource "azurerm_network_security_rule" "allow_https2" {
  name                        = "Allow-HTTPS2"
  priority                    = 120
  direction                   = "Outbound"
  access                      = "Allow"
